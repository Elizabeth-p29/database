...
class SchoolBase:
    def __init__(self, students_file, teachers_file):
    ...
    def load_data(self):
        students_df = pd.read_csv(self.students_file, header=None)
        students_df.columns = ["StLastName", "StFirstName", "Grade", "Classroom", "Bus"]
        students_df["Grade"] = students_df["Grade"].astype(int)
        students_df["Classroom"] = students_df["Classroom"].astype(int)
        students_df["Bus"] = students_df["Bus"].astype(int)

        teachers_df = pd.read_csv(self.teachers_file, header=None)
        teachers_df.columns = ["TLastName", "TFirstname", "Classroom"]
        teachers_df["Classroom"] = teachers_df["Classroom"].astype(int)

        merged = pd.merge(students_df, teachers_df, on="Classroom", how="left")

        self.students = merged
        self.teachers = {}
    ...
    def search_by_teacher(self, last_name):
        ...
        for (t_last, t_first), students in self.teachers.items():
            if t_last and t_last.lower() == last_name.lower():
                found = True
...
database = SchoolBase(students_file, teachers_file)
